...
.DESCRIPTION
    ...
    FreeType is built twice, as libfreetype.a and the unoptimised libfreetype_D.a that
    QSanguosha.pro links for debug configurations; the other archives have no debug
    variant in any of the platforms the repository ships.
...
function Compile-Sources {
    ...
    # Debug archives keep symbols and skip optimisation, matching the libfreetype_D.a
    # files the other platforms ship. Both branches assign a complete array: a
    # single-element array returned by an if statement is unwrapped to a string, and
    # string + array concatenates every flag into one argument.
    $baseFlags = if ($Configuration -eq 'debug') {
        @('-g', '-O0', '-fPIC', '-fno-strict-aliasing', '-w')
    } else {
        @('-O2', '-fPIC', '-fno-strict-aliasing', '-w')
    }
...
# QSanguosha.pro selects this variant with
# "CONFIG(debug, debug|release): LIBS += -lfreetype_D", and the Windows, Linux, macOS
# and 32-bit Android trees all ship it. The arm64 tree is generated by this script,
# so it has to produce the same archive or a debug Android build cannot link.
$freetypeDebugObjects = Compile-Sources -Name 'freetype_D' -Sources $freetypeSources `
    -IncludeDirs @((Join-Path $freetypeSrc 'include')) `
    -Defines @('FT2_BUILD_LIBRARY') `
    -Std 'gnu89' -Configuration debug `
    -ObjectRoot $objRoot -Compiler $tools.Cc -ThrottleLimit $Jobs

New-StaticArchive -ArchivePath (Join-Path $arRoot 'libfreetype_D.a') `
    -Objects $freetypeDebugObjects -Archiver $tools.Ar
...
foreach ($name in 'libfreetype.a', 'libfreetype_D.a', 'libogg.a', 'libvorbis.a', 'libvorbisfile.a') {
...
foreach ($name in 'libfreetype.a', 'libfreetype_D.a', 'libogg.a', 'libvorbis.a', 'libvorbisfile.a') {
    ...
    $minimum = if ($name -like 'libfreetype*.a') { 35 } else { 1 }
